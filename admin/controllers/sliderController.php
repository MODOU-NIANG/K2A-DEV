...
if (isset($_POST['editType'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $url_image = "documentheques/" . $code_type . "/" . $nom_image;
        $query = "UPDATE `types` SET `titre`='$titre', `description`='$description', `image`='$url_image', `sous_titre`='$sous_titre' WHERE `id`='$id_type'";

        // Supprimer l'ancienne image
        deleteFile($existingFilePath);
    } else {
        $query = "UPDATE `types` SET `titre`='$titre', `description`='$description', `sous_titre`='$sous_titre' WHERE `id`='$id_type'";
    }
